...
library(heemod, quietly = TRUE)
library(logOfGamma, quietly = TRUE)
library(ggplot2, quietly = TRUE)
library(survminer, quietly = TRUE)
library(officer, quietly = TRUE)
library(officedown, quietly = TRUE)
library(magrittr, quietly = TRUE)
library(Hmisc, quietly = TRUE)
library(future.apply, quietly = TRUE)
library(crosstable, quietly = TRUE)
library(flextable, quietly = TRUE)
library(stringr, quietly = TRUE)
library(BCEA, quietly = TRUE)
...
#
# - If you have 8GB of RAM on your computer, you can run this model with 2
# cores only but it may even be faster to run in series if you have other
# things open on your computer at the same time. Therefore, please set
# keep_free_cores to NA and run the model in series. This is because when the
# RAM on your computer runs out your computer will use the hard-disk instead
...
# - if you are using a HPC, you should be able to run this model with many
# cores due to the typically large amount of RAM available per core in a HPC
#
#
...
# This variable is used throughout the model to define whether to provide
# additional outputs useful for QC or not The model will take longer to run
...
#### These functions are used to extract parameters from the Excel input
### workbook for use in R # During Phase 2 a Shiny front-end will be added to
### the model which will allow an alternative mechanism to upload these types of
### inputs
...
#### Takes as an input the defined sequences, evaluation type and line to start
### the evaluation from ## Other input is % receiving each subs therapy at
### each line dependent on previous treatments received Reweights so that the
#### % receiving each treatment sums to 100% within each arm / line being
### studied # Outputs a matrix that has the % receiving each possible
### combination
...
## by treatment, line, population and outcome fitted survival curves using
## Flexsurvreg (exp, Weibull, lognormal, loglog, Gompertz, gen gamma)
...
# inputs for the model, NOT the parameters used to calculate the model. In
# effect, this is a place to store all model information BEFORE it gets boiled
# down to what's needed to run 1 model.
#
...
# This model allows two possible structures to be analysed: state transition
# with a user definable number of lines with health states based on time to
# discontinuation (drug costs) and progression status (quality of life and
...
# You will need to manually select the inputs file relevant to your user type,
# this is not stored on Github as access to CIC information differs by user
# type

# The first part of this code pulls all of the named ranges from the excel
# workbook, expand the parameters table

# Option to define Excel path on local machine - comment in this and comment out
# the code below to select file
...
# We then create a place for identifiers. Adding in an object to i full of
# lookup tables makes automated translation possible even when one doesn't know
# the number of items ex ante, or how they combine.
#
# If the lookup table is correct one can translate id numbers to text strings
# which are consistent throughout the entire model. This is extremely useful as
# the model can be expanded to any number of treatments and potentially even
# any number of lines (up to a reasonable maximum)

i$id <- list(ipd = list())
i$lookup <- list(ipd = list())

# Add distribution names to i
# This model only includes standard parametric distributions as more complex
# distributions were not deemed to be required for the included treatments
...
# The next step is to then "tidy up" i into another object, p. p doesn't
# necessarily have to house everything, only things that will change in PSA
...
# This code produces a list of possible sequences per population based upon the
# rules defined for RCC and the user input number of lines
...
# Uncomment this code to view the sequences and write the sequences defined
# to csv
...
# Pull out the raw data from the IPD excel book - one named range per treatment
# at each line Each reference curve is defined in Excel as time (weeks),
# event/censor (event coded as 1, censor as 0), patient group, line, molecule,
# trial and endpoint Pull all of the named ranges from the excel workbook,
# expand the parameters table
...
# - trial (trial id WITHIN population line and molecule to set them apart from
#   each other - usually just 1!)
# - endpoint

# These numeric identifiers are then used to create a nested list of survival
# regression models and extrapolations. The extrapolations are filtered down to
# the extrapolations that are selected within the excel input sheet, but the
# rest are kept here in i in case of scenario analysis.
#
# Note that the lookup tables in the next section are used to translate these
# numbers into human-readable identifiers.

# 3.3.2 Data identification ------------------------------------------

# There is a lot of nesting involved in this part of the analysis, with
# population line, regimen trial and endpoint making a total of 5 layers of
# nesting to automatically go through each endpoint for each trial for each
# regimen for each line for each population, perform all regression analyses,
# produce parameters and have an easily identifiable (and therefore
# programmable) spaces for the results of each analysis which can then be spat
# out into reporting.

# The first step is to break up r_pld into separate datasets depending on the
# identifiers. A function is used to do this which returns nothing if such data
# for one id set doesn't exist.
#
# Note that at this stage it is just those contexts which HAVE got PLD which
# are to be organised. For those endpoints and so on that do not have data, a
# separate step after this one to populate every endpoint for every treatment
# line for every treatment sequence is performed.
...
# One can also do the opposite, translating input file descriptions into
# numbers:
...
# This then means that one can translate the treatment sequence data generated
# earlier into numerical versions in one go:

# Start by making the id for population fit with the rest of the model (pop_
# with pop starting from 0). NOTE that there is 1 more population in treatment
# sequences than in the rest of the model...
...
# The "clean" version of sequences - first with words, then with numbers, then
# references
...
# Lots of nesting is present, however this simplifies upon explanation: lapply
# on a data.frame or data.table goes column-wise, so going across columns
# substitute the values for the values in i$lookup$trt which have corresponding
# names, returning the numbers which are consistent throughout the model. The
# way of looking inside e.g. network is e.g. pop_2$line_5$mol_2$endpoint_1, so
# now we can use the tables produced below to "order" the inputs for a
# treatment pathway
...
#
# This is automatically in line with the reference tables in the excel front
# end loaded at the time. If the ordering is changed there it needs updating in
# the IPD and in the lookup tables in the lists sheet of excel (and throughout
# excel!)
#
#
#
...
# on all pop line mol trial endpoint combinations with available data and
# return NULL for the rest

# Let's  perform some labelling like we did for treatment sequences for
# convenience/QC
i$surv$lab_pld <- list()
...
# Note to debug it is very helpful to set verbose to TRUE below so that you can
# identify the datasets which are problematic (e.g. not converging, 0 time
# values)
...
# The below code runs the survival analysis and saves as an RDS file for
# upload, this will only run if you set i$dd_run_surv_reg to "Yes" either in
# the Excel input or here
...
if (i$dd_run_surv_reg == "Yes") {
  ...
  i$surv$reg$pop_0$line_4$mol_999$trial_2$endpoint_4 <- lapply(1:1, function(x) {
    ...
    # curly braces on their own mean do this stuff and only return the last
    # thing or what's in a return call
    plot <- {
      # First the IPD is produced in a format that survminer will accept. Data
      # must all be the same format with the same column names. This assumes no
    ...
    # Now that we've done everything for this dataset, return a list of the
    # stuff we need for it:
    return(list(
...
# option to load from pre-specified file path on local machine, uncomment this
# and comment out the line below to use
...
# So that's all of the TSD14 survival analysis done. The next step is to
# programmatically proliferate comparative efficacy



# On a tablet with very little computational power this takes a couple of
# minutes to run. on a new laptop its not long at all
...
# Where the "weibull" part would come from a dropdown list in the Excel
# front-end of the model specific to that endpoint for that treatment for that
# line for that population (i.e. a lot of selections need to be made!)


# The stuff inside of i$surv$reg can be used to automatically populate a report
# presenting the full plot, regression summaries, goodness-of-fit results and
# the fit of the selected (via excel) distribution. The output can then be
# manually appended to include written justification for the selection(s) to
# drastically reduce the overhead associated with reporting survival analysis
# results and decisions made.


# 3.3.4 Survival analysis reporting ---------------------------------------

# the next step is to go through all of the results based directly on survival
# data and produce a readout containing:
#
# - Regression summary tables
# - goodness of fit
# - extrapolations (short and long-term) for visual fit assessment
#
# Each of these should have a separate section which at least states the
# identifiers (i.e., translating from numbers to text as in Section 3.4.2
# above)
#
# The best way to do this is with either Reduce or base for loops:
#

# Produce all the KM, extrapolations and gof tables for decisions on the
# front-end

# Note whether or not the survival analysis report is run by the code is set in
# Excel as this takes a long time to produce This cannot be produced without
# access to PLD
...
# Pull in the data and calculate means by pop line mol endpoint reftrt and
# reftrial
...
# Option to read in PH NMA CODA from local machine, uncomment this and comment
# out the line below to use
...
# option to read in from local machine, uncomment the below and comment out
# line 949 to use
...
# P is for the parameters for one model scenario. the relative efficacy network
# is required in order to compute the s(t) for all the different PLMTEs we need
# to power the model with. Therefore, the samples which are used should go into
# p not i.
#
...
# Turn this into a list structure using the same naming convention as the rest
# of the model:
#
#
# For this population, population line molecule trial and endpoint, generate a
# list of spaces containing information on the relationship between other
...
# A function (or functions) is (are) required to do several things, IN THIS
# ORDER:
#
#  1. Put the HRs from the PH NMA in the destinations, using the CODA sample
#     identifiers to set the origin.
#  2. Put the time-varying HRs from the FP NMA in the destinations, using the
#     identifiers to set the origins
#  3. Use the table R_table_eff_data_settings from Excel to apply any
#     superseding & any assumption/ad-hoc stuff (HRs, assume equal to and so
#     on)
#  4. Use the final network object to propagate relative efficacy throughout
#     the network, producing a set of extrapolations with RE applied.
#
...
# Use the information we have from the PHNMA CODA sample to populate the
# corresponding places within p$releff$network
#
...
# A QC check: All treatments are included in the network uncomment to check:
...
# survival analysis applied to the data. HOWEVER, we need those rows in the
# table as well as we need the "origin" distributional selections to fill in
# our output!
...
# Apply the setting dd_use_PHnma_for_FPnma from excel. This supersedes what's
# selected in efficacy settings, changing all FP NMA entries to PH NMA to force
# the model to use the PH NMA CODA sample over and above the FP NMA!
...
# Now that we have collected together all of the survival extrapolations for
# all of the data we have, and all of the HRs, whether by assumption or derived
# via NMA, plus all of the underlying distributional selections, we can proceed
# to "proliferate" the evidence network, starting from the extrapolations we
# have already and working "outwards" from that point, arriving at ONLY the
# extrapolation for each possibility (rather than all distributions). From
# there, we can "cherry-pick" those population line molecule trial endpoint
# distribution HR application choices to "build-up" a given treatment pathway.
...
#
# Because this is for a particular iteration of the model, it should go into
# object p which is the input set for running one model.
#
#
...
# Active trial depends on excel settings. Therefore this test is not automated.
# Instead, please use the below examples to get the "head" or top 6 rows of
# a specific set of survival outcomes. The argument `len` determines how many
# rows to return. See the definition of f_qc_surv_gethead to see the code.
...
# This subsection focuses on applying the required modifications to the "raw"
# extrapolations now that we have propagated comparative efficacy. We need to
# first apply any adjustments for treatment effect waning and then check the
# following:
#
...
# During technical engagement functionality will be added here to adjust for
# the impact of prior adjuvant treatment as part of scenario analysis
...
# Curve crossing should be fixed on the treatment sequence level, NOT the
# original PLMT level through E. This is because doing it that way round could
# lead to implausible extrapolations. For instance, if the PFS from somewhere
# else was being injected in (e.g. if trial 0 for OS and trial 1 from a
# different PLMT for PFS via equivalence assumption), it may be the case that
# the assumed equal PFS crosses OS.
#
# Therefore, curve crossing fixes should be performed on the individual
# treatment pathways.
#
# In conclusion, this section ONLY adjusts for gpop mortality, and curve
# crossing adjustment is performed AFTER f_seq_extrapCollector is used to pull
# through the correct extrapolations to the correct DESTINATION locations.
#
# The function is ONLY applied to OS lines.
#
# Because we have pulled through the metatadata to our PLMTEs s(t) objects, we
# can use the "dest" element to decide which row of Excel range R_table_ptchar
...
# Note this function produces a warning which details when there is a danger of
# TE waning in the usual fashion (setting hazards the same) being implemented
# producing counter-intuitive results. In this case we use the higher of the
# hazards to prevent this problem
...
p$surv$gpop <- if (i$dd_age_sex_source == "Mean") {
  f_surv_GenOSLines_det(
...
# During QC model we produce a comparison between mean-based and IPD based
# general population OS lines:
if (qc_mode) {
  i$gpop <- list(
...
p$surv$st <- f_surv_PFSxOS(p$surv$st, if (i$dd_adj_cross_curves == "Use hazards") {
  "hazardmax"
} else {
  "abs"
})

p$surv$st <- f_surv_TTDxOS(p$surv$st, if (i$dd_adj_cross_curves == "Use hazards") {
  "hazardmax"
} else {
  "abs"
})
...
# after all relative efficacy has been applied. This creates a LOT of graphs.
# You can look through them by pressing the arrow on the plots window
if (qc_mode) {
...
# Utilities are applied to the disease model by treatment by line and whether
# the patient is on or off treatment Age adjustment is conducted
# multiplicatively in line with DSU guidance using earlier defined patient
# characteristics for age and sex
...
if (i$dd_ageadjuutilities == "Yes") {
  if (i$dd_age_sex_source == "Mean") {
    ...
    p$util$gpop <- lapply(ptc_L1, function(pop) {
      adjust_utility(
        age            = pop$age,
        sex            = pop$sex,
...
i$QALYs$utilities$means <- f_process_utilities(
...
# compared to baseline as well as our HSUVs by PLM and treatment status.
#
...
# the method to apply AEs (one-off or per cycle) is defined in the Excel inputs
# file this applies to both costs and QALYs

# options related to the source of AE data are defined in the Excel workbook
# the functions below read in the information required to produce AE costs and
# utilities per cycle, this includes AE durations and the trial durations
# associated with the rates which are used to produce one off cost and QALY
# impacts
...
# Drug and admin costs are applied per treatment per line (as drug costs may
# differ depending on what line treatment is used at) The impact of stopping
# rules is consider as part of the calculation of drug and admin costs rather
# than in determining whether patients in the on or off treatment health states
#
# MRU costs are applied per treatment and by on and off treatment status as the
# EAG was advised that MRU is the same across different lines When this model
# is expanded to a generic version flexibility to define per line will be added
# One off costs are included for treatment initiation at each line of
# treatment, terminal care at the end of life (applied on death) and
# progression (radiotherapy and surgery costs)
...
# These are then added to the first element of the cost vectors in the patient
# flow function.

# holding line for PSA - this will be replaced by function sampling from means
# and SEs
...
# Read in cost and QALY consequences for subsequent treatments per first line
# option from Excel This information is only used whaen the PartSA model
...
if (!str_trim(i$dd_model_struct) %in% c("State transition", "Partitioned survival")) {
  stop(
...
# Running the model can mean two different structures, a markov model which we
# refer to as state transition or a partitioned survival approach. These both
# work using the same parameters object p. There is only one function to
# compute patient flow, f_pf_computePF. This takes several different arguments
# but the main one is p, or the parameters object to power the model with.
# Everything within f_pf_computePF uses objects which are within p, such that a
# copy of p could be saved as a file and as long as all functions have been
# defined and packages loaded the model can be run directly from there.

# The only structures allowed are state transition and partitioned survival.
# error if it is not one of those:
...
# Depending on which model structure is used to compute patient flow, the
# results are processed in different ways. The below function simply routes the
# pf object to the appropriate function to process the results, and returns an
# object res containing the results:
...
if (p$basic$structure == "State transition") {
  population_numbers <- if (sum(p$basic$pops_to_run == 1:3) > 0) {
    1:3
  } else {
    1:6
  }
  res$mk$qaly_shortfall_1_to_3 <- lapply(population_numbers, function(npa_pop) {
    ...
    out <- calc_severity_modifier(
      age = bl_age,
      sex = bl_male,
      .patient_level = if (i$dd_age_sex_source == "Mean") {
        FALSE
      } else {
        TRUE
      },
...
# Outputting the results to word requires a series of functions to produce
# component parts to go in the report, and then consolidating them all into the
# requested output.
...
# and automatically name it using the variables that are already inside of i
# and p (including structure, which is located in p$basic$structure. Note that
# this comes from i$dd_model_struct, i.e. named range dd_model_struct from
# excel. please ensure that this is updated for PS model scenarios):
