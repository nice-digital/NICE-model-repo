...
## Comparative efficacy propagation - or, proportional hazards network meta analysis (PHNMA)
...
PH NMA CODA samples. <mark>TODO: understand what "CODA samples" are</mark>
...
Looking at example from first row of table, you can see it has populated `hr` and `orig` using `p$releff$CODA$PH`.
...
These results it enters are, as explained in `Model_Structure.R`, the "fractional polynomial point estimate time-varying hazard ratios". It also notes that "we only need to apply this one to the stuff that is NOT direct survival analysis applied to the data. HOWEVER, we need those rows in the table # as well as we need the 'origin' distributional selections to fill in our output!"

```{r}
head(p$releff$network$pop_0$line_1$mol_4$trial_0$endpoint_0$fp$HR, 50)
```
...
And checks if it matches one of the "flags" in `i$List_eff_datasources`. These are refered to as the:

* **Trial flag** - `Trial survival analysis` - uses HR from patient-level data (PLD) extrapolation. In the function, it notes that "the user in excel has indicated that this is one of the reference curves, overriding everything else. # Origin should be the same as destination"
* **FPNMA flag** - `FP_NMA` - "HRs are linked corresponding to the CODA sample, but overrides are to be enacted. CODA sampling is done externally"
* **PHNMA flag** - `PH_NMA` - "time-varying HRs are linked corresponding to the input data, but overrides are to be enacted. sampling done within."
* **ET flag** - `Assume equal to` - set HR to 1
* **AHR flag** - `Apply HR to` - "put in origin and set HR to the "HR to apply" column in the table. also simple. for PSA use bounds externally, replacing the values in the table"

<mark>TODO: don't understand this all yet</mark>

```{r}
i$List_eff_datasources
```

We can view the hazard ratios using `f_releff_extract_all_HRs()`:

```{r}
f_releff_extract_all_HRs(p$releff$network)$pop_0$line_1
```
...
Creates `p$surv$st`. <mark>TODO:don't understand function yet, very complicated</mark>

From `Model_Structure.R`...
...
Not run in default case... <mark>TODO: understand</mark>
